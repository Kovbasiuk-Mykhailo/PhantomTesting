...
casper.test.begin( 'Unblock testsuite', function ( test ) {
	...
	phantomcss.init( {
		...
		screenshotRoot: fs.absolute( fs.workingDirectory + '/testsuites/LoginState/help/screenshots' ),
		failedComparisonsRoot: fs.absolute( fs.workingDirectory + '/testsuites/LoginState/help/failures' ),
			...
			this.waitForSelector('body > app > popups > div > popup-email-signin', function(){
				...
				this.waitFor(function() {
						...
						this.waitForSelector("body > app > popups > div > popup-email-signin > div > div.input-container > v-form > div:nth-child(2) > div > input", function() {
							...
							waitForSelector("body > app > slide-overlay > div", function () {
								phantomcss.screenshot("body > app > slide-overlay > div", 'help');
							});
						});
					});
			});
